...
分类器只需要统一的消息对象。平台鉴权、权限范围和分页留在适配器里。当前注册 `feishu` 和 `wecom`；模板不代表其他平台已实现。企业微信的实际支持范围见 WECOM.md。
...
3. Add the source option in `dist/index.html`. In `dist/app.js`, add the provider label and connection instructions. Search and message requests use the selected provider ID; identities and queues are separated by source. Review provider-specific status and link behavior before enabling it.
